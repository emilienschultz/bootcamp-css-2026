...
1. The new era of scientific computing
...
## New skills in computational social science?
...
Knowing their words (*pod, bucket, job, image, quota*) is what makes a ticket answered in one exchange instead of five.
...
- One script = one step (if you want to go further, a `run.sh` / `Makefile` chains them-
- Parameters as CLI arguments or a config file, never hard-coded (so you can share your code)
...
- Conventions > tools: write them in the README
...
| 1 GB | one Wikipedia shard, compressed | think about RAM |
...
TO DO : CREATE A VM + PUT THE IP

```bash
ssh user@SERVER                      # connect
scp file.py user@SERVER:~/project/    # copy a file
```
...
- `htop`, `nproc`, `free -h`, `df -h`, `du -sh *`: know the machine
- Keep a job alive after disconnecting: `nohup python script.py &` or **`tmux`**
- Reach a remote Jupyter: `ssh -L 8888:localhost:8888 crest`
- Run your pipeline there: `git clone https://github.com/ORG/repo.git && ./run.sh`
...
4. Use it as the password when Git asks, and let Git remember it:
...
git clone https://github.com/ORG/group-X.git  # username + token as password
```

::: {.callout-warning}
The token is stored in clear in `~/.git-credentials` on the pod. It dies with the pod. Never paste it in a file of the repository.
:::
...
```bash
python src/build_dataset.py --subset 20250702.en --keyword data --delete-raw \
   --shards train_00000.parquet
```
...
for i in $(seq -w 0 16); do
    python src/build_dataset.py --subset 20250702.en --keyword data --delete-raw \
        --shards train_000$i.parquet
done
python src/merge.py   # concatenate the per-shard outputs on S3 into one built dataset
```
...
The same treatment, without pandas. Each of these tools **streams** by design.

| Tool | What it is | When |
|---|---|---|
| **HF CLI** (`hf`) | download from the terminal | scripts in bash |
| **`mc`** | S3 client in the terminal | move files without Python |
| **DuckDB** | SQL engine on files (CLI + Python) | look at a parquet, aggregate, filter |
| **polars** | a lazy DataFrame library | a pandas replacement that scales |
| **xan** | CSV toolkit in Rust | huge CSVs from the shell |

## Download and upload from the terminal

```bash
pip install -U huggingface_hub
hf download omarkamali/wikipedia-monthly 20250702.en/train_00013.parquet \
   --repo-type dataset --local-dir ./data/raw

mc cp data/processed/wiki_data.parquet s3/group-x/wiki/processed/
mc ls s3/group-x/wiki/processed/
```

Compare with `build_dataset.py`: what does the Python script add? (the downsizing, arguments, idempotence, logging)
...
### xan (CSV)
```bash
duckdb -c "COPY (SELECT id, title, url, text FROM '$F') TO 'wiki16.csv';"
xan count wiki16.csv
xan search -s text data wiki16.csv | xan select id,title,url | xan count
xan map 'len(text) as length' wiki16.csv | xan hist -s length
```

:::

Run one of them on the 2.3 GB shard, on the 4 GB service. Does it hold?
